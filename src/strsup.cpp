...
sint_t AString::GetXMLAttribute(const char *attr, AString& str, sint_t start) const
{
	AString smarker1, smarker2, emarker;
	sint_t pos, len;

	smarker1.Format("<%s>", attr);
	smarker2.Format("<%s ", attr);
	emarker.Format("</%s>", attr);

	if ((pos = GetFieldPos(smarker1, emarker, start, &len)) >= 0) {
		str = Mid(pos, len);
	}
	else if ((pos = GetFieldPos(smarker2, emarker, start, &len)) >= 0) {
		while (pText[pos] && (pText[pos] != '>')) {
			if ((pText[pos] == '\"') || (pText[pos] == '\'')) {
				char c = pText[pos];

				pos++;
				len--;
				while (pText[pos] && (pText[pos] != c)) {
					if ((pText[pos] == '\\') && pText[pos + 1]) {
						pos += 2;
						len -= 2;
					}
					else {
...
bool AString::SetXMLAttribute(const char *attr, const char *value, sint_t start)
{
	AString smarker, emarker;
	AString value1 = value;

	smarker.Format("<%s>", attr);
	emarker.Format("</%s>", attr);

	return SetField(smarker, emarker, value1, start);
}

AString AString::GetXMLAttributes() const
{
	AString str;
	int i, j;
	...
	for (i = j = 0; (i < len());) {
		if (pText[i] == '<') {
			...
			if (i > j) str += Mid(j, i - j) + "\n";

			j = i;

			i++;
			while (pText[i] && (pText[i] != '>')) {
				if		(pText[i] == quote) quote = 0;
				else if ((pText[i] == '\'') || (pText[i] == '\"')) quote = pText[i];
				else if (pText[i] == '\\') i++;
